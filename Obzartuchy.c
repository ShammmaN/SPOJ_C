...
int main(){
    ...
    unsigned int howMany, player, biscuitsInBox=0;
    unsigned long int playerTime=0;
    unsigned int eatenBiscuits=0, boxes=0;
    ...
    while (howMany>0)
    {
        eatenBiscuits=0;
        scanf("%d",&player);
        scanf("%d",&biscuitsInBox);

        while (player>0)
        {
            scanf("%d", &playerTime);
            eatenBiscuits+=day/playerTime;
            player--;
        }
    
         if(eatenBiscuits%biscuitsInBox==0)  
         {
            boxes = eatenBiscuits/biscuitsInBox;   
         }   
    
         else 
            boxes = (eatenBiscuits/biscuitsInBox)+1;
